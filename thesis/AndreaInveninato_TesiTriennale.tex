...
\begin{figure}[t]
    ...
    \caption{\textit{Euclid} region of interest (RoI) in an all-sky Mollweide projection. The blue borders enclose the $\SI{16000}{deg^2}$ RoI that contains the observed sky of the Euclid Wide Survey. The RoI excludes the Galactic and ecliptic planes. Further details can be found in~\cite{EuclidPaper}.}
    \label{fig:EuclidRoI}
\end{figure}
...
Figure~\ref{fig:ClusteringWedgesPlot_measVScorr_z1vsz2} illustrates the clustering wedges weighted by the squared separation $s^2\xi_\mathrm{p}(s)$ to better visualize them (as they would otherwise take on low values), comparing the correct clustering signal against the measured one.
...
To get a better view these differences, figure~\ref{fig:ClusteringWedgesPlot_ratio_z1vsz2} depicts the direct ratio of the measured to the correct signal.
...
By following the procedures described in Sec.~\ref{sec:BayesianInference}, we have obtained the posterior distributions of the free parameters for both models, CLEFT and CLPT, for each redshift and for every scale cut considered. The posterior plots are shown in figures~\ref{fig:CLEFT_smin20_z1vsz2}, ~\ref{fig:CLEFT_smin30_z1vsz2} and ~\ref{fig:CLEFT_smin40_z1vsz2} for the CLEFT model with $s_\mathrm{min} \in \{ 20,30,40 \} \,h^{-1} \mathrm{Mpc}$ and in figures~\ref{fig:CLPT_smin20_z1vsz2} and ~\ref{fig:CLPT_smin30_z1vsz2} for the CLPT model with $s_\mathrm{min} \in \{ 20,30 \} \,h^{-1} \mathrm{Mpc}$.
...
Our analysis has revealed that interlopers do not affect the clustering of the Universe in an isotropic way. In the perpendicular wedge, interlopers act predominantly as an unclustered background, uniformly suppressing the amplitude of the correlation without altering its underlying geometric shape. In contrast, in the parallel wedge, line-of-sight mapping errors severely distort the correlation function, flattening crucial features such as the peak of the Baryonic Acoustic Oscillation (BAO). Since the multipolar expansion is integrated over the entire angular domain, these line-of-sight distortions propagate across all multipolar moments, fundamentally corrupting the overall signal.
