...
\titleformat{\chapter}[hang]
  {\normalfont\fontsize{22}{25}\bfseries}
  {\thechapter\quad}
  {0pt}
  {}
...
\end{equation}
where the angle brackets $\langle \, \dots \rangle$ denote an average over different realizations of the Universe. Under the assumption of ergodicity, this ensemble average can be evaluated as a spatial average over a sufficiently large volume of the Universe.
...
\paragraph{Multipole expansion}\label{parag:MultipoleExpansion}
The most common technique to compress the angular dependence of $\xi(s, \mu)$  is to expand the function into a series of Legendre polynomials, $\mathcal{P}_\ell(\mu)$. The anisotropic $\xi(\mu, s)$ may be written as
\begin{equation}
  \xi (\mu, s) = \sum_{\ell \in \mathbb{N}_0} \mathcal{P}_\ell (\mu) \xi_\ell(s) \; ,
  \label{eq:MultipoleExpansion}
\end{equation}
...
To entirely eliminate the line-of-sight distortions, one can define the projected correlation function, $w_\mathrm{p}(r_\mathrm{p})$,  constructed by integrating $\xi(r_p, \pi)$ along the entire parallel separation axis:
\begin{equation}
  w_\mathrm{p}(r_\mathrm{p}) \equiv 2 \int_{0}^{\infty} \xi(r_\mathrm{p}, \pi) \mathrm{d} \pi \; .
  \label{eq:ProjectedFuncion}
\end{equation}
In practice, however, the upper integration limit cannot extend to infinity. Instead, the integral is truncated at a finite maximum separation, $\pi_\mathrm{max}$, to prevent the accumulation of large-scale noise from regions where the correlation signal is weak or unconstrained.


\chapter{Euclid}\label{chap:Euclid}
Launched in July 2023, \textit{Euclid} is a medium-class space telescope in the Cosmic Vision 2015-2025 program of the European Space Agency (ESA)~\cite{EuclidPaper}. Covering a redshift range up to $z \sim 2$ and executing a wide-field survey across approximately $\SI{14000}{deg^2}$ of the extragalactic sky, the mission aims to understand the nature of dark energy, dark matter, and the validity of the $\Lambda\mathrm{CDM}$ on cosmological scales.
...
\begin{itemize}
  ...
  \item \textbf{Euclid Deep Survey (EDS)}: Covering approximately $\SI{40}{deg^2}$ spread across three specialized patches of the sky. This survey reaches a flux limit approximately $6.3$ times lower than that of the Wide Survey. This enhanced sensitivity allows EDS to serve as a vital calibration tool to quantify selection effects and systematic uncertainties in the main catalog.
\end{itemize}
By observing millions of active, star-forming galaxies across these expansive fields, \textit{Euclid} effectively samples the density contrast field $\delta(\mathbf{x})$ defined in Section~\ref{sec:LSStructureStatistics}, and thus transforms these raw observations into the 3D comoving coordinates required for our Landy-Szalay clustering calculations.

\subsection{Slitless Spectroscopy}\label{subsec:SlitlessSpectroscopy}
Traditional spectrographs isolate individual target galaxies using physical masks with narrow slits or fiber-optic cables. Although this method provides sharp, isolated spectra, their implementation in a space mission involves a certain degree of mechanical complexity due to the need for moving parts to adjust the position of the slits. For this reason, \textit{Euclid} uses slitless spectroscopy: a technique that employs a “grism” (a combination of a prism and a grating) to produce a dispersed image of a field in the sky without the use of apertures. Unlike slit spectroscopy, it does not isolate objects within the field, thus yielding a two-dimensional spectrum that is a convolution of the object’s physical shape and its spectral properties. With this technique, all sources are captured simultaneously, resulting in spectral contamination due to source overlap being one of the main challenges. To address this issue, different dispersion angles are used to separate the spectra in orthogonal directions. Initially, \textit{Euclid} planned to use three red grisms with dispersion angles of $\SI{0}{\degree}$, $\SI{180}{\degree}$ and $\SI{270}{\degree}$; however, performance issues with the $\SI{270}{\degree}$ grism led to the adoption of an alternative method, in which the remaining two grisms are rotated by $\SI{4}{\degree}$, providing a total of four different dispersion directions.
...
\begin{figure}[t]
    ...
    \caption{\textit{Euclid} region of interest (RoI) in an all-sky Mollweide projection. The blue borders enclose the $\SI{16000}{deg^2}$ RoI that contains the observed sky of the Euclid Wide Survey. The RoI excludes the Galactic and ecliptic planes. Figure taken from~\cite{EuclidPaper}.}
    \label{fig:EuclidRoI}
\end{figure}
...
\paragraph{The Near-Infrared Spectrometer and Photometer}\label{parag:NISP}
The spectroscopic channel of the Near-Infrared Spectrometer and Photometer (NISP) onboard \textit{Euclid} enables the simultaneous acquisition of thousands of slitless spectra across its field of view with uniform quality. For EWS, the spectroscopic configuration (NISP-S) utilizes four $\SI{140}{\milli\meter}$ diameter grisms treated with multilayer dielectric coatings to provide sharp out-of-band blocking. This prevents unwanted ultraviolet, optical, or mid-infrared background photons from entering the spectroscopic channel and contaminating the target passband of $\num{1206}\mathrm{-}\SI{1892}{\nano\meter}$. Designed to detect $\mathrm{H}\,\alpha$ emitters within the redshift range $0.84 < z < 1.88$, these grisms provide a dispersion of $\SI{1.372}{\nano\meter/pixel}$ and a resolving power of approximately $R \approx 480$ for a $\SI{0.5}{\degree}$ diameter source, meeting the mission requirement of $R > 380$ to achieve a redshift accuracy of $\sigma(z) < 0.001(1 + z)$. This dimensionless metric quantifies the instrument's capacity to distinguish two closely spaced points or adjacent signals as separate entities and it is defined as the ratio of the operating wavelength to the minimum resolvable wavelength interval:
\begin{equation}
...
The phenomenon of gravitational lensing, predicted by Einstein’s theory of General Relativity, occurs when light from distant galaxies is bent as it passes through the curved spacetime nearby massive objects. In some cases, the bending of the light is so significant that multiple images of the galaxy are formed; this is typically called strong gravitational lensing. In other cases, the light bending is subtle, and the images of galaxies are distorted, stretched and magnified in small amounts; this is named weak gravitational lensing~\cite{WeakLensing}. Although this gravitational lensing effect physically alters the shape of each individual galaxy, the resulting deformation is significantly dependent on the galaxy’s intrinsic ellipticity. Consequently, the weak gravitational lensing signal cannot be isolated from a single object due to this dominant shape noise. Instead, it must be extracted statistically by averaging over a large set of galaxies; assuming that the intrinsic orientations of the galaxies are random, this averaging cancels out the stochastic shape noise, revealing the coherent alignment induced by the foreground gravitational tidal field.

By combining this information with measurements of the redshift distribution of the sources, we can trace the evolution of the large-scale structure (LSS), thereby allowing us to probe the effects of dark energy. \textit{Euclid} will achieve this, by acquiring the shapes of approximately $1.5$ billion galaxies through VIS and supplemented by ground-based observations.

\paragraph{Galaxy clustering}\label{parag:GalaxyClustering}
This probe uses galaxies as discrete luminous tracers of the underlying, inhomogeneous dark matter distribution. By measuring the three-dimensional spatial coordinates of tens of millions of galaxies, GC allows us to compute the two-point clustering statistics. From these statistics, we extract two major cosmological features: Baryon Acoustic Oscillations (BAO), which serve as a standard ruler to chart the expansion history $H(z)$, and Redshift-Space Distortions (RSD)~\cite{EisensteinBAO}, which track the growth rate of cosmic structure to test theories of modified gravity. To achieve this, \textit{Euclid} uses NISP’s slitless spectroscopy to measure the redshift of nearly $35$ million galaxies in the range $0.84 < z < 1.88$.
...
Despite dark energy making up about $\SI{68}{\percent}$ of the present-day energy density, we are unsure whether it is an actual cosmological constant, a field that evolves dynamically, or reflects a more profound modification of the gravitation laws at cosmological scales. To distinguish between such models we can analyze the way the equation-of-state parameter $w(z)$ varies with redshift. We adopt the Chevallier-Polarski-Linder (CPL)~\cite{CP_param}~\cite{L_param} parametrization
\begin{equation}
...
Any measured deviation from this value would point out a breakdown of Einstein’s equations on large scales. By measuring redshift-space distortions in the galaxy clustering statistics, \textit{Euclid} aims to evaluate $\gamma_\mathrm{g}$ with a precision better than $0.02$ (at $\SI{68}{\percent}$ confidence), providing a definitive test of General Relativity on cosmological scales.

\section{Redshift determination}\label{sec:RedshiftDetermination}
As the target galaxies are located at cosmological distances $0.84 < z < 1.88$, their primary optical emission lines are shifted by cosmic expansion into the near-infrared band.  The primary spectral feature targeted by \textit{Euclid} for redshift determination is the $\mathrm{H}\,\alpha$ (Hydrogen-alpha) emission line, which has a rest-frame wavelength of $\lambda_\mathrm{rest} = \SI{656.3}{\nano\meter}$.

However, the previously mentioned medium-low spectral resolution, $R \approx 480$, results in the $\mathrm{H}\,\alpha$ line and the N\,\textsc{ii}~$\lambda\lambda 654.9,658.4 \,\mathrm{nm}$ doublet, where the double-lambda symbol conventionally denotes the plural wavelengths of a closely spaced pair of transitions, being blended into a single emission feature that cannot be separated at the SNR selection threshold of the \textit{Euclid} spectroscopic sample. Moreover, since the limited wavelength range and SNR generally prevent the detection of multiple emission lines, most redshift assignments have to rely on a single-line detection to assign a redshift value.

Once an emission line is detected and successfully identified as $\mathrm{H}\,\alpha$, its observed wavelength $\lambda_{\text{obs}}$ is measured, and the redshift $z_\mathrm{obs}$ of the galaxy is computed directly via Eq. \eqref{eq:Redshift_def}. Using this redshift, the galaxy is assigned a radial distance coordinate using the comoving distance relation, Eq. \eqref{eq:ComovingDistance} for $D_\mathrm{c}(z)$, adjusting its position in the 3D map.

\subsection{The origin of interlopers}\label{subsec:TheOriginOfInterlopers}
Line misidentifications inevitably lead to the introduction of interlopers in the catalogues, that is galaxies that have been mistakenly identified. In cases where the observed wavelength of a spectral feature is interpreted as the rest-frame wavelength of the incorrect line, the relationship Eq. \eqref{eq:Redshift_def} shifts such that
\begin{equation}
  \frac{1 + z_\mathrm{true}}{1 + z_\mathrm{meas}} = \frac{\lambda_\mathrm{wrong}}{\lambda_\mathrm{true}} \; ,
  \label{eq:Redshift_interlopers}
\end{equation}
where $\lambda_\mathrm{true}$ is the true rest-frame wavelength of the photon when it was emitted by the gas in the distant galaxy and  $\lambda_\mathrm{wrong}$ is the rest-frame wavelength the computer assumes it is looking at, that is $\mathrm{H}\,\alpha$. In case of interlopers, this ratio significantly differs from one.

Efficient statistical silulations, described in~\cite{IlariaR}, help us identy interloper galaxies in the \textit{Euclid} spectroscopic sample. These simulations show that [O\,\textsc{iii}]~$\lambda 500.8 \,\mathrm{nm}$ and [S\,\textsc{iii}]~$\lambda 953.1 \,\mathrm{nm}$ forbidden lines (hereafter [O\,\textsc{iii}] and [S\,\textsc{iii}]) are the primary sources of redshift errors. The [O\,\textsc{iii}] line is visible at $1.5 < z_\mathrm{true} < 2.7$. Within $1.5 < z_\mathrm{true} < 1.8$, misidentification is less likely because both $\mathrm{H}\,\alpha$ and [O\,\textsc{iii}] are detectable. Following the relationship in Eq. \eqref{eq:Redshift_interlopers}, since [O\,\textsc{iii}] has a shorter wavelength than $\mathrm{H}\,\alpha$, these interlopers happen to have $z_\mathrm{meas} < z_\mathrm{true}$, resulting in an actually further away galaxy compared to their estimated distance. On the other hand, [S\,\textsc{iii}] is detectable at $0.3 < z_\mathrm{true} < 0.94$, which is overlaps for a short range with the one of $\mathrm{H}\,\alpha$. Therefore, the same argument applies to [S\,\textsc{iii}]: as it has a larger wavelength than $\mathrm{H}\,\alpha$, the software places these galaxies too far away and, in reality, they are closer to us than estimated.

In addition to these so-called “line interlopers”, we expect to observe “noise interlopers”. The latter ones consist of spurious detections where noise fluctuations in low-SNR, featureless spectra mimic valid emission lines. These contaminants are typically faint stars or galaxies with spectral features that are either inherently weak or outside the instrument's wavelength range. Unlike line interlopers, there is no correlation between the true and measured redshifts because the selection relies entirely on random spikes; thus, we expect a rather uniform distribution of these kind of interlopers across different wavelengths and redshifts.

\chapter{Methodology and Data analysis}\label{chap:MethodologyDataAnalysis}
\section{Dataset}\label{sec:Dataset}
In this analysis, we utilize the \textit{EuclidLargeMocks} (ELM)~\cite{ELM}, a dedicated suite of numerical simulations designed to mimic the survey area of the first Data Release (DR1) spectroscopic sample of the EWS. The galaxy catalogues extracted from these simulations are lightcones with an angular footprint on the sky of a circle with radius $\SI{30}{\degree}$ and spanning the redshift range $0 < z_\mathrm{true} < 3$ with the total area of the cone being $\SI{2763}{deg^2}$.

These catalogues provide a basic set of information for each galaxy: sky coordinates, true redshift, including peculiar velocities and $\mathrm{H}\,\alpha$ line flux. To assign measured redshifts ($z_{\mathrm{meas}}$) in ELM, a probabilistic model calibrated on end-to-end 1D spectral simulations (FastSpec/OU-SPE; Granett et al., in prep.) is used. This model defines the conditional probability density function $P(z_{\mathrm{meas}}|z_{\mathrm{true}})$ as a Gaussian mixture: a narrow component with $\sigma_{0,z} = 0.001$ represents correct galaxies and (suitably rescaled) line interlopers, while a broad component represents noise interlopers. Sources are then classified based on this sampled value: they are tagged as “correct” if $|z_{\mathrm{meas}} - z_{\mathrm{true}}| < 3\sigma_{0,z}$, as “line interlopers” if within $3\sigma_{0,z}$ of an expected interloper redshift, and as “noise interlopers” otherwise. Because the broad noise distribution overlaps with the narrow components, the probability of a noise interloper falling within $5\sigma_{0,z}$ of $z_{\text{true}}$ is non-negligible. For further details on the construction of the interloper contribution in the mocks see Risso et al.~\cite{IlariaR}. While adjusting the classification thresholds could mitigate this overlap, it remains fundamentally impossible to distinguish truly correct galaxies from noise interlopers. The so-defined mean fractions of interlopers in the ELM for all considered spectroscopic redshift bins are inserted in the following table:
\begin{table}[ht]
    \centering
    \begin{tabular}{|c|c|c|c|c|}
        ...
        \hline
        [O\,\textsc{iii}] & 0.03 & 0.12 & 0.09 & 0.01 \\
        \hline
        [S\,\textsc{iii}] & 0.01 & 0.03 & 0.08 & 0.07 \\
        \hline
        ...
        \hline
      \end{tabular}
      \caption{Mean fractions of the diﬀerent interloper types in the four spectroscopic redshift bins in the contaminated EuclidLargeMocks~\cite{IlariaR}.}
...
\begin{itemize}
  \item the “correct”, clean dataset: it represents the ideal, true cosmological distribution of galaxies, which still incorporate the statistical error due to the measurement uncertainty. In this sample, the assigned spectroscopic redshifts correspond to the true positions of the galaxies, entirely free from line misidentifications; 
  \item the “measured”, contaminated dataset: it contains realistic observational noise and line confusion effects. It includes the simulated populations of [O\,\textsc{iii}] and [S\,\textsc{iii}] interlopers discussed in Section~\ref{subsec:TheOriginOfInterlopers}.
\end{itemize}

We make use of a set of $1000$ independent mock realizations of both datasets in order to smooth out fluctuations across individual mock catalogues. For each of the $1000$ catalogues in both the measured and correct paths, the raw data vectors include:
\begin{itemize}
...
For each of the $1000$ independent realizations in both the measured and correct datasets, we collect the previously mentioned data.
However, before evaluating the Landy-Szalay estimator, we first apply a spatial rebinning procedure to the spatial grid. The native comoving separation grid, $s$, within the simulations is highly finely sampled. Computing the 2PCF at this native resolution would introduce significant shot noise due to the small number of galaxy pairs within each interval. To suppress these statistical fluctuations and optimize SNR to levels consistent with the expected observational uncertainties, the spatial grid is rebinned into $40$ bins in $s$. On the other hand, the angular bins in $\mu$ are kept identical to the original $200$ binning.

Following this rebinning, the raw pair counts are appropriately normalized using the catalog sizes to obtain $\mathrm{DD}$, $\mathrm{DR}$, and $\mathrm{RR}$. The rebinned arrays are then used to compute the Landy-Szalay estimator for each individual mock catalogue using Eq.~\ref{eq:LSEstimator}.

\section{Wedges and Multipoles}\label{sec:WedgesAndMultipoles}
Because the full two-dimensional $\xi(s, \mu)$ grid contains thousands of highly correlated data points, utilizing it directly for cosmological parameter inference is inefficient. Therefore, the information must be compressed into one-dimensional statistics: clustering wedges and multipole expansion, as described in Sec.~\ref{subsec:ClusteringStatistics}.

In order to obtain the most accurate and statistically rigorous estimate of these observables, we do not compute the summary statistics on the overall mean of the 2PCF. Instead, we compute both the wedges and multipoles independently for each of the $1000$ individual mocks first, and only then take the ensemble mean of those vectors. This order of operations prevents the non-linear propagation of errors and ensures that the variance across the realization is accurately represented.

\paragraph{Clustering wedges estimation}\label{parag:ClusteringWedgesEstimation}
Clustering wedges are constructed by averaging the 2PCF over broad angular intervals in $\mu$. For every individual mock file $i$, we project the rebinned $\xi_i(s, \mu)$ into parallel and perpendicular wedges as in Eq.~\eqref{eq:ClusteringWedges}.
Once this projection is complete for all realizations, the final observable is derived by taking the arithmetic mean over the ensemble: $\bar{\xi}_\mathrm{p}(s) = \langle \xi_{\mathrm{p}, i}(s) \rangle$, where $\mathrm{p}$ representes the parallel or perpendicular wedge.

\paragraph{Multipole expansion estimation}\label{parag:MultipoleExpansionEstimation}
The full anisotropic information is compressed by projecting the $\xi_i(\mu, s)$ grids onto the basis of Legendre polynomials, $\mathcal{P}_\ell(\mu)$, to extract the multipole moments as in Eq.~\eqref{eq:MultipoleExpansion}. We compute here the first three even multipoles: the monopole ($\ell = 0$), the quadrupole ($\ell = 2$), and the hexadecapole ($\ell = 4$). Similar to the wedges, these multipoles are calculated for each of the $1000$ realizations and then compute the mean of the multipoles to yield the final data vector $\bar{\xi}_\ell(s) = \langle \xi_{\ell, i}(s) \rangle$.
...
\section{Non-linear models}\label{sec:NonLinearModels}
To extract cosmological parameters from our clustering statistics, we must compare the measurements against a robust theoretical prediction of the 2PCF. On very large scales, standard linear perturbation theory provides an adequate description of structure formation. However, at the intermediate and non-linear scales probed by our $s_{\mathrm{min}}$ cuts ($20$, $30$, and $40 \,  h^{-1} \mathrm{Mpc}$), linear theory breaks down due to three primary physical effects: the non-linear gravitational collapse of matter, the complex kinematics of redshift-space distortions and galaxy bias. To accurately model these scales and preserve the Baryon Acoustic Oscillation feature, our parameter inference pipeline relies on two analytical models: Convolution Lagrangian Perturbation Theory (CLPT) and Convolution Lagrangian Effective Field Theory (CLEFT).

\subsection{Convolution Lagrangian Perturbation Theory}\label{subsec:CLPT}
Introduced by Carlson et al. (2013)~\cite{CLPT}, CLPT is built entirely within the Lagrangian framework of fluid dynamics, extending foundational formulations of standard Lagrangian Perturbation Theory~\cite{Buchert}. Instead of tracking density fluctuations at fixed coordinates in space, as in the Eulerian approach, the Lagrangian framework tracks the trajectories of individual cosmic fluid elements over time. The core concept of LPT model relies on mapping the comoving distance $\mathbf{x}$ from its initial Lagrangian coordinate $\mathbf{q}$ via a displacement field $\boldsymbol{\Psi}$:
\begin{equation}
  \mathbf{x} (t) = \mathbf{q} + \boldsymbol{\Psi}(\mathbf{q}, t) \; .
  \label{eq:CLPT}
\end{equation}

The pioneering and simplest iteration of this framework is the Zel'dovich approximation~\cite{Zeldovich}, which corresponds to first-order LPT. In this regime, the displacement field is assumed to be purely linear and proportional to the initial gravitational acceleration, successfully modeling the formation of large-scale structures (such as cosmic webs and filaments).

Beyond the linear regime, however, standard LPT expansions treat higher-order corrections perturbatively, causing the theory to rapidly break down at intermediate, quasi-linear scales. CLPT tries to fix this by executing a non-perturbative partial resummation of the displacement field's correlation functions~\cite{Matsubara}. It keeps the terms that remain constant at large spatial separations exponentiated, effectively smoothing out small-scale displacement fluctuations. By doing this, at its lowest perturbative order, CLPT naturally reduces to the Zel'dovich approximation, while its higher-order corrections yield highly accurate descriptions of galaxy clustering and redshift-space distortions well into the quasi-linear regime.

\subsection{Convolution Lagrangian Effective Field Theory}\label{subsec:CLEFT}
Introduced by Vlah, Castorina and White (2016)~\cite{CLEFT}, CLEFT represents an significant step forward that combines the mathematical tools of CLPT with the principles of Lagrangian Effective Field Theory (LEFT)~\cite{Porto}.

All standard perturbation theories inevitably fail at small spatial scales (the non-linear regime). This is due to highly non-linear gravitational collapse, shell-crossing, and complex baryonic physics (such as supernova or Active Galactic Nuclei feedback) that cannot be modeled from first principles. Adopting the EFT paradigm~\cite{Baumann}, CLEFT treats this small-scale, short-distance physics by systematically “integrating out” the unresolvable small-scale fluctuations from the macroscopic equations of motion.

The backreaction of these non-linear scales onto the larger, perturbative regimes is encapsulated by introducing effective counterterms into the equations. These counterterms introduce a set of free parameters that embed the unknown small-scale dynamics. By fitting these parameters to observational data or simulations, CLEFT successfully absorbs the small-scale uncertainties, thereby extending its predictive validity to significantly smaller physical scales than those of pure CLPT.

\section{Bayesian inference}\label{sec:BayesianInference}
At this stage, we use a Bayesian inference algorithm to determine the cosmological parameters. The code is written in Python and consists of two main blocks: a likelihood evaluation block and a parameter space sampler. The likelihood computation is built upon the infrastructure of the COMET emulator~\cite{CometRef}, which has been specifically modified and extended to incorporate the non-linear theoretical predictions of both the CLPT and CLEFT models. This extension to COMET was first introduced and applied in \cite{MartinK}. On the other end, the algorithm used to explore the parameter space is the Nautilus sampler~\cite{Nautilus}.
...
\begin{equation}
  ...
  \label{eq:Posterior}
\end{equation}
$\mathbf{D}^\mathrm{data}$ is our experimental data vector, evaluated independently for the measured and correct datasets; for the wedge-based inference, we use the mean perpendicular wedge vector, $\bar{\xi_\perp} (s)$; for the multipole-based inference, it is, instead, the concatenation of the monopole, quadrupole, and hexadecapole of the 2PCF $[\bar{\xi}_0(s), \bar{\xi}_2(s), \bar{\xi}_4(s)]$. $\mathbf{D}^\mathrm{theo}$ is the corresponding theoretical vector generated by the COMET library for a given parameter set $\theta$, operating under either the CLPT or CLEFT model. Lastly, $C^{-1}$ denotes the inverse covariance matrix, also known as the precision matrix. Because the underlying covariance matrix $C_{\mathrm{sample}}$ is estimated from a finite ensemble of $N_{\mathrm{mocks}} = 1000$ independent realizations, its direct inversion introduces a systematic bias. To achieve an unbiased estimator for the precision matrix, we scale the inverse sample covariance matrix by the Hartlap correction factor~\cite{Hartlap_2006}
\begin{equation}
  C^{-1} = \frac{N_{\mathrm{mocks}} - d - 2}{N_{\mathrm{mocks}} - 1} \, C_{\mathrm{sample}}^{-1} \; ,
  \label{eq:HartlapFactor}
\end{equation}
where $d$ is the dimension of the data vector $\mathbf{D}^{\mathrm{data}}$.
...
The Figure of Merit is used to quantify the statistical ability of theoretical models to constrain cosmological parameters. We use the generalisation to higher-dimensional parameter spaces by Wang (2008)~\cite{FoM_Wang} defined as
\begin{equation}
...
\begin{figure}[!ht]
    ...
    \end{minipage}
    \caption{Plot showing the clustering wedges ratio between the measured and the correct datasets for the redshift bin $z \in [0.9,1.1]$ in the left slot and $z \in [1.1,1.3]$ in the right slot. The grey horizontal line represents the attenuation factor $(1 - f_\mathrm{tot})^2$.}
...
\begin{figure}[!ht]
    ...
    \end{minipage}
    \caption{Plot showing the monopole and quadrupole ratio between the measured and the correct datasets for the redshift bin $z \in [0.9,1.1]$ in the left slot and $z \in [1.1,1.3]$ in the right slot. The grey horizontal line represents the attenuation factor $(1 - f_\mathrm{tot})^2$.}
...
Fig.~\ref{fig:ClusteringWedgesPlot_measVScorr_z1vsz2} illustrates the clustering wedges weighted by the squared separation $s^2\xi_\mathrm{p}(s)$ to better visualize them (as they would otherwise take on low values), comparing the correct clustering signal against the measured one.
The perpendicular wedge shows both the measured and correct signals to have an identical  shape. The overall amplitude of the measured signal is suppressed uniformly across all scales, yet the distinct features of the correlation function, for example the Baryon Acoustic Oscillation peak localized at $s \approx 100 \ h^{-1} \mathrm{Mpc}$, remain well-preserved in position and width. This is what we could expect for the perpendicular wedge as the contamination from interlopers in a slitless survey primarily causes a radial displacement of galaxies along the line-of-sight, thus leaving their angular positions in the sky intact. On the other hand, the parallel wedge, rather than a simple drop in amplitude, shows a considerable difference in shape between the measured signal and the correct one. While the correct signal features a pronounced BAO peak, the interloper-contaminated signal shows a significant smoothing of these acoustic features: the measured parallel wedge is visibly flattened overall.

To get a better view these differences, Fig.~\ref{fig:ClusteringWedgesPlot_ratio_z1vsz2} depicts the direct ratio of the measured to the correct signal.
In the perpendicular wedge, the ratio remains nearly constant and stable at around the factor $(1 - f_\mathrm{tot})^2$, represented in the plot as a grey horizontal line, across a broad spatial range ($0 < s < 120 \, h^{-1}\mathrm{Mpc}$). This constant behavior implies that transverse interlopers behave predominantly as an unclustered, random background population.  On the contrary, the parallel wedge ratio reveals a highly complex, scale-dependent profile: this highlights how errors in line-of-sight mapping artificially redistribute the clustering power across different radial scales, significantly attenuating the acoustic characteristics.

Above separations of $s > 130 \, h^{-1} \mathrm{Mpc}$, the ratios for both wedges exhibit substantial statistical fluctuations accompanied by rapidly expanding error bars. This behavior is a numerical artifact of the vanishing amplitude of the correlation function at large separations. As $\xi_\mathrm{p}^{\mathrm{correct}}(s)$ naturally approaches zero on these cosmic scales, the ratio becomes highly sensitive to the slightest statistical fluctuations in the numerator, leading to the observed numerical instability.

\paragraph{Multipole expansion}
Fig.~\ref{fig:MultipolesPlot_measVScorr_z1vsz2} displays the re-scaled multipoles, $s^2\xi_{\ell}(s)$, comparing the correct and measured distributions for $\ell \in \{ 0,2,4 \}$. The monopole incorporates distortions from all directions. The measured monopole shows a clear systematic suppression of amplitude compared with the corrected signal across all spatial scales examined. Furthermore, the acoustic peak at $s \approx 100 \, h^{-1}\mathrm{Mpc}$ is visibly broadened and attenuated under the influence of the intruders. At large distances ($s > 120 \, h^{-1}\mathrm{Mpc}$), both profiles pass through zero, although the measured profile remains less pronounced than the correct one. The quadrupole is the first-order indicator of cosmic anisotropy determined by redshift space distortions. The correct quadrupole  manifests a characteristically deep negative amplitude, which reaches a minimum near $s \approx 55 \, h^{-1}\mathrm{Mpc}$, together with a BAO feature at $100 \, h^{-1}\mathrm{Mpc}$. On the other hand, the measured signal undergoes strong attenuation, essentially cancelling out the BAO feature and thus showing that radial mapping errors drastically smooth out the apparent directional clustering traces. Lastly, the higher-order anisotropies detected by the hexadecapole fluctuate around zero due to their low intrinsic amplitude. Nevertheless, a systematic damping is still evident, as the measured hexadecapole shows attenuated variations compared with its corrected counterpart.

The scale dependence of these distortions is quantified in Fig.~\ref{fig:MultipolesPlot_ratio_z1vsz2} using the multipole ratios, $\xi_{\ell}^{\mathrm{measured}} / \xi_{\ell}^{\mathrm{corrected}}$, for the monopole and quadrupole moments. Unlike the purely constant ratio observed in the transverse clustering wedge, the monopolar ratio shows a distinct, though slight, dependence on scale. This behaviour suggests that, since the monopole integrates over components parallel to the line of sight, it cannot be modelled as a simple scale-independent attenuation factor. The quadrupole ratio reveals a systematic tilt: the ratio decreases continuously in respect to the scale. This downward trend implies that contamination from interlopers suppresses anisotropic features on larger scales more markedly than those on smaller scales.

\section{Impact on parameters}\label{sec:ImpactParamaters}
By following the procedures described in Sec.~\ref{sec:BayesianInference}, we have obtained the posterior distributions of the free parameters for both models, CLEFT and CLPT, for each redshift and for every scale cut considered. The posterior plots are shown in Figs.\ref{fig:CLEFT_smin20_z1vsz2}, ~\ref{fig:CLEFT_smin30_z1vsz2} and ~\ref{fig:CLEFT_smin40_z1vsz2} for the CLEFT model with $s_\mathrm{min} \in \{ 20,30,40 \} \,h^{-1} \mathrm{Mpc}$ and in Figs.\ref{fig:CLPT_smin20_z1vsz2} and ~\ref{fig:CLPT_smin30_z1vsz2} for the CLPT model with $s_\mathrm{min} \in \{ 20,30 \} \,h^{-1} \mathrm{Mpc}$.

The most obvious systematic error lies in the estimation of the clustering amplitude, $A_\mathrm{s}$. Across both models and all $s_\mathrm{min}$ cuts, the measured posterior consistently yields a underestimated $A_\mathrm{s}$ compared to the correct value for both clustering statistics, wedge and multipoles. This behavior is a direct consequence of the scale-independent signal attenuation observed in the transverse clustering wedge: as interlopers suppress the overall amplitude of the correlation function by a factor $(1 - f_\mathrm{tot})^2$, the model incorrectly interprets this purely observational artifact as a universe with intrinsically weaker matter clustering. The 2D contours for $A_\mathrm{s}$ versus the linear bias $1+b_1$ depicts a strong degeneracy: a characteristic elongated shape. Because interlopers suppress the overall intensity, the model attempts to compensate shifting the constraints along this degeneracy axis, resulting in artificially distorted measurements of both the cosmological amplitude and the galactic bias parameters.

Overall, the 1D marginal distributions for $h$ and $\omega_\mathrm{c}$ remain reasonably centered on their true values (indicated by the dashed lines), even for the measured data.
Going into detail, the physical density of cold dark matter, $\omega_\mathrm{c}$, presents slight shifts in its posterior contours. Interlopers do not simply reduce the amplitude of the signal; anisotropic clustering also distorts the shape of the correlation function, in particular suppressing and broadening the peak of BAO along the line-of-sight. This distortion of the shape disrupts theoretical models, forcing shifts in $\omega_c$ as the model attempts to fit the modified broadband shape of the 2PCF. This effect is particularly noticeable for the multipoles.
On the other end, the Hubble parameter, $h$, is more robust since we do not observe any strong discrepancy with the fiducial value in any of our cases.

We can see that, for the galaxy bias and small-scale correction parameters, the inclusion of interlopers does not result in a significant shift in the parameter estimates, as the posteriors for both clustering statistics - the plots derived from the measured and correct data - have a similar shape and differ only slightly in width. It is, however, worth noting that, in several cases, the 1D posterior distributions become highly non-Gaussian, strongly asymmetric, or even reach the limits of their uniformly distributed prior intervals. This means that, through Bayesian inference of these clustering statistics using these models, we did not obtain a meaningful constraint on these parameters.

To determine whether the systematic errors introduced by interlopers are limited to small-scale, non-linear regimes or whether they have a global impact on the dataset, we varied the minimum scale threshold of the data vector. Specifically, the CLEFT model was evaluated at $s_{\mathrm{min}} = 20, \,30, \,40 \, h^{-1}\mathrm{Mpc}$, whilst the CLPT model was tested at $s_{\mathrm{min}} = 20, \,30 \, h^{-1}\mathrm{Mpc}$.
Comparing the posterior distributions across these progressive scale cuts reveals two distinct effects. As $s_{\mathrm{min}}$ is increased from $20$ to $40 \, h^{-1}\mathrm{Mpc}$, a broadening of both the 1D and 2D posterior distributions is observed for all parameters. This increase in uncertainties is a direct consequence of the exclusion of highly information-rich small-scale galaxy pairs, which substantially reduces the overall signal-to-noise ratio available to the sampler. It is important to note, however, that the systematic shift in the primordial amplitude $A_\mathrm{s}$ does not decrease with larger-scale cuts: even at $s_{\mathrm{min}} = 40 \, h^{-1}\mathrm{Mpc}$, the measured contours remain significantly shifted and completely disconnected from the correct reference values. Therefore, this shows that contamination by interlopers cannot be regarded as a localised, small-scale modelling problem. On the contrary, it acts as a systematic effect that lowers the clustering amplitude and distorts the geometry of the correlation function across all physical scales.
...
\begin{figure}[t]
    ...
    \end{minipage}
    \caption{Contour plots for CLEFT model with scale-cut $s_\mathrm{min}=20 \, h^{-1}\mathrm{Mpc}$ for the redshift bin $z \in [0.9,1.1]$ in the left slot and $z \in [1.1,1.3]$ in the right slot. The grey dashed lines represent the fiducial values for the cosmological parameters.}
...
\begin{figure}[!ht]
    ...
    \end{minipage}
    \caption{Contour plots for CLEFT model with scale-cut $s_\mathrm{min}=30 \, h^{-1}\mathrm{Mpc}$ for the redshift bin $z \in [0.9,1.1]$ in the left slot and $z \in [1.1,1.3]$ in the right slot. The grey dashed lines represent the fiducial values for the cosmological parameters.}
...
\begin{figure}[!ht]
    ...
    \end{minipage}
    \caption{Contour plots for CLEFT model with scale-cut $s_\mathrm{min}=40 \, h^{-1}\mathrm{Mpc}$ for the redshift bin $z \in [0.9,1.1]$ in the left slot and $z \in [1.1,1.3]$ in the right slot. The grey dashed lines represent the fiducial values for the cosmological parameters.}
...
\begin{figure}[!ht]
    ...
    \end{minipage}
    \caption{Contour plots for CLPT model with scale-cut $s_\mathrm{min}=20 \, h^{-1}\mathrm{Mpc}$ for the redshift bin $z \in [0.9,1.1]$ in the left slot and $z \in [1.1,1.3]$ in the right slot. The grey dashed lines represent the fiducial values for the cosmological parameters.}
...
\begin{figure}[!ht]
    ...
    \end{minipage}
    \caption{Contour plots for CLPT model with scale-cut $s_\mathrm{min}=30 \, h^{-1}\mathrm{Mpc}$ for the redshift bin $z \in [0.9,1.1]$ in the left slot and $z \in [1.1,1.3]$ in the right slot. The grey dashed lines represent the fiducial values for the cosmological parameters.}
    \label{fig:CLPT_smin30_z1vsz2}
\end{figure}

\subsection{Performance metrics}\label{subsec:PerformanceMetrics2}
Once the posterior distributions have been obtained, we can apply the performance metrics, presented in Sec.~\ref{sec:PerformanceMetrics}, in order to quantify and compare the ability of the two models to correctly estimate the cosmological parameters and to provide accurate and precise constraints as a function of the scale-cut. Fig.~\ref{fig:FoMFoB} provides a comprehensive summary of the relative performance of the models in a single plot.

The top row displays the FoM. Since it is inversely proportional to the parameter uncertainties, a higher FoM signifies a tighter, more statistically precise posterior distribution. Conversely, the bottom row displays the FoB, which quantifies the multi-dimensional distance between the mean values of the inferred parameters and their true, fiducial values and thus describes the accuracy of the obtained parameters. The dark and light grey shaded regions mark, respectively, the $\SI{68}{\percent}$ confidence region and the $\SI{95}{\percent}$ confidence one.
...
On the other hand, the perpendicular wedge shows significant stability against contamination from interlopers: for both models and for all redshifts, the FoB remains within the acceptable threshold, even on non-linear scales ($s_\mathrm{min} = 20 \ h^{-1}\mathrm{Mpc}$), without losing too much in terms of precision.

These behaviours are consistent in both the CLEFT and CLPT models, as well as across both redshift ranges. This consistency indicates that the robustness of the transverse wedge with respect to multipoles is not a localised artefact of a single perturbation theory framework or a specific cosmic epoch, but rather a fundamental geometric characteristic of the way in which angular wedges isolate contamination parallel to the line of sight.

\chapter{Conclusions}\label{chap:Conclusions}
In this thesis we have introduced and investigated the prominent systematic error in modern spectroscopic galaxy clustering pipelines: interloper contamination. The final goal of this work was to evaluate the impact of these errors onto cosmological parameter constraints.
...
Our analysis has revealed that interlopers do not affect the clustering of the Universe in an isotropic way. In the perpendicular wedge, interlopers act predominantly as an unclustered background, uniformly suppressing the amplitude of the correlation without altering its underlying geometric shape. In contrast, in the parallel wedge, line-of-sight mapping errors severely distort the correlation function, flattening crucial features such as the peak of the Baryonic Acoustic Oscillation (BAO). Since the multipole expansion is integrated over the entire angular domain, these line-of-sight distortions propagate across all multipole moments, fundamentally corrupting the overall signal.

These morphological distortions have a direct impact on the constraints on cosmological parameters. For both models, the signal attenuation caused by the intruder leads to a systematic underestimation of the primordial clustering amplitude, $A_\mathrm{s}$. By carrying out a test with different minimum scale cut-off thresholds ($s_{\mathrm{min}} = 20, \,30,\, 40 \, h^{-1}\mathrm{Mpc}$), we have shown that this parameter distortion is not a small-scale localised modelling error, but rather a systematic overall effect. Masking the non-linear scales merely increases the statistical uncertainties by discarding informative galaxy pairs, without resolving the underlying offset relative to the true fiducial cosmology.

Ultimately, we found a compromise between statistical precision and accuracy, quantified by the Figure of Merit and the Figure of Bias. Although the multipole method holds a large amount of statistical information, providing a FoM that is an order of magnitude higher than that of wedges, this precision is heavily biased: the resulting FoB drastically exceeds acceptable confidence thresholds. In contrast, the perpendicular clustering wedge depicts exceptional stability. Despite a loss in terms of statistical precision, the wedge method successfully isolates the analysis from line-of-sight contamination, keeping the FoB well below the acceptable threshold across all tested scale-cuts and redshift ranges, thus resulting in an accurate constraint.

\section{Future steps}\label{sec:FutureSteps}
In line with the overall objective of this work, a natural next step is to extend the pipeline to evaluate purely projected clustering statistics, such as the projected correlation function $w_\mathrm{p}(r_\mathrm{p})$. By mathematically integrating the entire radial separation axis along the line of sight, these statistics compress spatial information in such a way as to be conceptually immune to radial displacement errors. A comparison of the performance of $w_\mathrm{p}(r_\mathrm{p})$ with the perpendicular clustering wedge will determine whether the complete elimination of the line-of-sight axis provides a more robust framework for cosmological extraction and will help to quantify the exact loss of statistical precision resulting from the exclusion of all radial information.

A completely different approach might be, rather than attempting to avoid systematic bias by relying solely on geometric data compression, to model interloper contamination in advance, directly within the framework of perturbation theory. By analytically modifying the theoretical models of CLEFT or CLPT to incorporate a parameterised fraction of interlopers, such contamination could be treated as a dynamic perturbation parameter. During Bayesian likelihood sampling using the MCMC method, this parameter would be simultaneously constrained and marginalised.
